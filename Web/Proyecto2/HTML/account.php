<?php
	...
	if(empty($name)){
	  $nameError = 'Ingrese nombre de su cuenta';
	  $value = false;
	}

	if(empty($saldo)){
	  $saldoError = 'Ingrese una cantidad';
	  $value = false;
	}else if($saldo < 0){
	  $saldoError = 'No puede ingresar cantiades menores a 0';
	  $value = false;
	}

	if(empty($contrasena)){
	  $contrasenaError = 'Ponga su contraseña para confirmar';
	  $value = false;
	}else if (strcmp($contrasena, $contrasena_master)) {
	  $contrasenaError = 'La contraseña no es correcta, verifique';
	  $value = false;
	}

	if($value == true && $_POST['group'] == "agregar"){

		$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE saldo set saldo = saldo + ?  WHERE nombre_saldo = ? AND  usuario = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($saldo,$name,$_SESSION['usuario']));
			Database::disconnect();
			header("Location: account.php");
			exit();

	}else if($value == true && $_POST['group'] == "retirar"){
	...
	<div class="row">
		...
		<div class="col s6">
			...
			<form class="form-horizontal" action="account.php" method="post">
				...
				<div class="controls">
						<p>
							<input class="radio" name="group" type="radio" id="test1" value="retirar" checked/>
							<label for="test1">Retirar</label>
						</p>
						<p>
							<input class="radio" name="group" type="radio" id="test2" value="agregar"  />
							<label for="test2">Agregar</label>
						</p>
				</div>

				  <!-- Saldo de la cuenta-->
				<div class="control-group <?php echo !empty($saldoError)?'error':'';?>">
					<label class="control-label">Saldo a mover</label>
					<div class="controls">
