<?php
	...
	if ( !empty($_POST)) {
	  ...
	  if (empty($name)) {
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM saldo where nombre_saldo = ?";
		$q = $pdo->prepare($sql);
		...
		Database::disconnect();
		if($saldo  != 0){
		  $nameError = 'No pudedes cerrar cuentas que no esten en ceros';
		  $valid = false;
		}
	  }
	  ...
	  if ($valid) {
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "DELETE FROM saldo WHERE nombre_saldo = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($name));
			Database::disconnect();
...
<main>
	<div class="row">
		...
		<form class="form-horizontal" action="close_account.php" method="post">
